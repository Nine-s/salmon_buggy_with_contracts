process GENERATE_DECOY_TRANSCIPTROME {
    ...
    promise([FOR_ALL("f", ITER("*.fa"), { f -> IF_THEN(COND("grep -Ev '^[>ACTGUN;]' $f"), "exit 1") }), RETURN(NOT(EMPTY_FILE("gentrome.fa"))), COMMAND_LOGGED_NO_ERROR(), INPUTS_NOT_CHANGED()])
...
process SALMON_ALIGN_QUANT {
    ...
    promise([RETURN(NOT(EMPTY_FILE("transcripts_quant"))), COMMAND_LOGGED_NO_ERROR(), INPUTS_NOT_CHANGED()])
